...
// var shareEnable = true; //toggle share - Removed unused variable
var shareTitle = "Highscore on Liquid Sort is [SCORE]PTS"; //social share score title
var shareMessage =
  "[SCORE]PTS is mine new highscore on Liquid Sort Game game! Try it now!"; //social share score message
...
var gameData = {
  ...
  colorsArr: [],
  complete: false,
  inTransition: false, // Add this new property
  levelCompleted: 1,
  levelNum: 0,
  paused: true,
  stage: {},
...
var selectData = { column: 5, max: 20, page: 0, row: 4, total: 1 };
 ...
 *
 */
// function retrieveLevelData() { // Removed unused function
...
//       gameData.levelCompleted >= levelSettings.length
 ...
 *
 */
// function buildGameButton() { // Removed unused function
  ...
  buttonLevels.addEventListener("click", (evt) => {
    gameData.type = "level";
    playSound("soundButton");
    goPage("level");
  });
  ...
  buttonLevelArrowL.addEventListener("click", (evt) => {
    toggleSelect(false);
  });

  buttonLevelArrowR.cursor = "pointer";
  buttonLevelArrowR.addEventListener("click", (evt) => {
    toggleSelect(true);
  });

  buttonSelect.cursor = "pointer";
  buttonSelect.addEventListener("click", (evt) => {
    playSound("soundButton");
    goPage("game");
  });
  ...
  buttonArrowL.addEventListener("click", (evt) => {
    playSound("soundMatch");
    toggleTube(false);
  });

  buttonArrowR.cursor = "pointer";
  buttonArrowR.addEventListener("click", (evt) => {
    playSound("soundMatch");
    toggleTube(true);
  });

  itemExit.addEventListener("click", (evt) => {});
  ...
  buttonFacebook.addEventListener("click", (evt) => {
    share("facebook");
  });

  buttonTwitter.cursor = "pointer";
  buttonTwitter.addEventListener("click", (evt) => {
    share("twitter");
  });
  buttonWhatsapp.cursor = "pointer";
  buttonWhatsapp.addEventListener("click", (evt) => {
    share("whatsapp");
  });

  buttonSoundOff.cursor = "pointer";
  buttonSoundOff.addEventListener("click", (evt) => {
    toggleGameMute(true);
  });

  buttonSoundOn.cursor = "pointer";
  buttonSoundOn.addEventListener("click", (evt) => {
  ...
  buttonExit.addEventListener("click", (evt) => {
    togglePop(true);
    toggleOption();
  });

  buttonSettings.cursor = "pointer";
  buttonSettings.addEventListener("click", (evt) => {
    toggleOption();
  });
...
function toggleTube(con) {
  if (con) {
    gameData.tubeNum++;
    gameData.tubeNum =
      gameData.tubeNum > tubes_arr.length - 1 ? 0 : gameData.tubeNum;
  } else {
    gameData.tubeNum--;
    gameData.tubeNum =
      gameData.tubeNum < 0 ? tubes_arr.length - 1 : gameData.tubeNum;
  }
...
function selectTube() {
  ...
  gameData.tubes[0].scaleX = gameData.tubes[0].scaleY = 0.8;
  gameData.offsetY = (tubes_arr[gameData.tubeNum].fillH / 2) * 0.8;
}
...
function buildSelectLevel() {
  ...
  for (var r = 0; r < selectData.row; r++) {
    for (var c = 0; c < selectData.column; c++) {
      ...
      $.level[r + "_unlock_" + c].addEventListener("click", (evt) => {
...
function toggleSelect(con) {
  if (con) {
    selectData.page++;
    selectData.page =
      selectData.page > selectData.total ? selectData.total : selectData.page;
  } else {
    selectData.page--;
    ...
    for (var c = 0; c < selectData.column; c++) {
      ...
      if (startNum < gameData.levelCompleted) {
        if (
          ...
          gameData.levelNum + 1 === $.level[r + "_text_" + c].text
        ) {
...
function unlockLevelTween(r, c) {
  ...
  TweenMax.to($.level[r + "_unlock_" + c], 0.5, {
    alpha: 1,
    delay: 1,
    overwrite: true
  });
...
function goPage(page) {
  ...
  switch (page) {
    ...
    case "result":
      ...
      if (gameData.type === "level") {
        resultTitleTxt.text = textDisplay.resultLevelTitle.replace(
          "[NUMBER]",
      ...
      console.log("score updated:" + playerData.score);
      break;
  }
...
function saveGame(score) {
  ...
  /*$.ajax({
      type: "POST",
      url: `${window.location.origin}/api/games/save-results`,
      data: {score:score},
...
function positionTubes() {
  ...
  positionData.totalWidth =
    (positionData.totalCol - 1) * (positionData.width + gameData.marginX);
  positionData.totalHeight =
    (positionData.totalRow - 1) * (positionData.height + gameData.marginY);
  ...
  for (var n = 0; n < gameData.tubes.length; n++) {
    ...
    if (positionData.col > positionData.totalCol - 1) {
      ...
      if (leftTubes <= positionData.totalCol) {
        var lastRowWidth =
          (leftTubes - 1) * (positionData.width + gameData.marginX);
  ...
  var finalH =
    positionData.totalHeight + positionData.height + gameData.marginY;
  var maxWidth = 1000;
  var maxHeight = 500;
  var scaleX = 1;
  var scaleY = 1;
...
function fillAllTubes() {
  ...
  for (var n = 0; n < tubeArr.length; n++) {
    ...
    for (var l = 0; l < levelSettings[gameData.levelNum].levels; l++) {
      var heightNum = Math.floor(
        gameData.tube.fillH / levelSettings[gameData.levelNum].levels
      );
      storeColorArr.push({ color: colorIndex, height: heightNum });
    }
  }
...
function createTube(x, y) {
  ...
  if ($.editor.enable) {
    newLiquidContainer.data.imageBack = new createjs.Bitmap(
      tubeLoader.getResult("tubeBack" + gameData.tubeNum)
    );
    newLiquidContainer.data.imageFront = new createjs.Bitmap(
      tubeLoader.getResult("tubeFront" + gameData.tubeNum)
    );
  } else {
    newLiquidContainer.data.imageBack = new createjs.Bitmap(
      loader.getResult("tubeBack" + gameData.tubeNum)
    );
    newLiquidContainer.data.imageFront = new createjs.Bitmap(
      loader.getResult("tubeFront" + gameData.tubeNum)
    );
  }
  ...
  newLiquidContainer.data.mask.graphics
    .beginFill("#ccff00")
    .moveTo(
      tubes_arr[gameData.tubeNum].shape[0].x,
      tubes_arr[gameData.tubeNum].shape[0].y
    );
  for (var n = 0; n < tubes_arr[gameData.tubeNum].shape.length; n++) {
    if (tubes_arr[gameData.tubeNum].shape.length - n > 2 && isEven(n)) {
      newLiquidContainer.data.mask.graphics.curveTo(
        tubes_arr[gameData.tubeNum].shape[n + 1].x,
        tubes_arr[gameData.tubeNum].shape[n + 1].y,
        tubes_arr[gameData.tubeNum].shape[n + 2].x,
        tubes_arr[gameData.tubeNum].shape[n + 2].y
      );
    }
  }
  ...
  newLiquidContainer.addChild(
    newLiquidContainer.data.imageBack,
    newLiquidContainer.data.fillShape,
    liquidWaterContainer,
    liquidBottomContainer,
    liquidSurfaceContainer,
    newLiquidContainer.data.bubble,
    newLiquidContainer.data.imageFront
  );
  ...
  if (tubeEvent) {
    ...
    newLiquidContainer.addEventListener("click", (evt) => {
      if (gameData.paused || gameData.inTransition) {
      ...
      if (gameData.pouring[pourIndex].from === null) {
        if (evt.currentTarget.data.colors.length > 0) {
          ...
          waterContainer.setChildIndex(
            gameData.pouring[pourIndex].from,
            waterContainer.numChildren - 1
          );
          playSound("soundSelect");
        }
      } else if (
        gameData.pouring[pourIndex].from.data.index ===
        evt.currentTarget.data.index
      ) {
        ...
        if (
          ...
          var shapeIndex =
            gameData.pouring[pourIndex].from.data.colors.length - 1;
          var thisHeight =
            gameData.pouring[pourIndex].from.data.colors[shapeIndex].height;

          if (
            gameData.pouring[pourIndex].to.data.fill + thisHeight <=
            gameData.tube.fillH
          ) {
            moveTube(pourIndex);
          } else {
...
function mixSameLiquid(index) {
  ...
  for (var n = 0; n < thisLiquid.data.colors.length; n++) {
    if (n > 0) {
      if (
        thisLiquid.data.colors[currentIndex].index ===
        thisLiquid.data.colors[n].index
      ) {
        thisLiquid.data.colors[currentIndex].height +=
          thisLiquid.data.colors[n].height;
...
function fillLiquid(index) {
  ...
  var pos = { w: gameData.tube.colorW, x: 0, y: 0 };
  for (var n = 0; n < thisLiquid.data.colors.length; n++) {
    ...
    thisArray.shape.graphics
      .beginFill(colors_arr[thisLiquid.data.colors[n].index].fill)
      .drawRect(
        -(pos.w / 2),
        thisArray.shape.extraH,
        pos.w,
        -(thisArray.height + thisArray.shape.extraH)
      );
    ...
    thisArray.surface.graphics
      .beginFill(colors_arr[thisLiquid.data.colors[n].index].surface)
      .drawCircle(0, 0, gameData.tube.fillW / 2);
    thisArray.surface.color = thisLiquid.data.colors[n].index;

    thisArray.bottom = new createjs.Shape();
    thisArray.bottom.graphics
      .beginFill(colors_arr[thisLiquid.data.colors[n].index].fill)
      .drawCircle(0, 0, gameData.tube.fillW / 2);
    thisArray.bottom.color = thisLiquid.data.colors[n].index;

    thisArray.surface.scaleY = thisArray.bottom.scaleY =
      gameData.tube.surfaceScale;
    thisArray.surface.visible =
      n === thisLiquid.data.colors.length - 1 ? true : false;
...
function focusTube(obj, con) {
  ...
  TweenMax.to(obj, 0.2, { overwrite: true, y: posY });
}
...
function moveTube(pourIndex) {
  ...
  thisLiquid.data.direction =
    thisLiquid.x > thisFillLiquid.x ? "right" : "left";
  ...
  if (thisLiquid.data.direction === "left") {
    pos.x =
      thisFillLiquid.x -
      gameData.tube.fillX -
      thisLiquid.data.tubePos.x +
      thisLiquid.data.containerY;
  } else {
    pos.x =
      thisFillLiquid.x +
      gameData.tube.fillX -
...
function pourLiquid(pourIndex) {
  ...
  TweenMax.to(thisShape, gameSettings.fillSpeed, {
    ...
    onUpdate: () => {
      ...
      thisShape.graphics
        .clear()
        .beginFill(colors_arr[colorIndex].fill)
        .drawRect(
          -(gameData.tube.colorW / 2),
          thisShape.extraH,
          gameData.tube.colorW,
          thisShape.fillH - thisShape.extraH
        );
  ...
  thisFillLiquid.data.fillShape.graphics
    .clear()
    .setStrokeStyle(gameData.tube.pourW, "round", "round")
    .beginStroke(colors_arr[colorIndex].fill)
    .mt(0, 0)
    .lt(0, -gameData.tube.pourH);

  TweenMax.to(thisFillShape, gameSettings.fillSpeed, {
    ease: Linear.easeNone,
    fillH: thisShape.fillH,
    onComplete: () => {
      thisFillLiquid.data.colors[thisFillLiquid.data.colors.length - 1].height =
        Math.abs(thisFillShape.fillH);
    },
    onUpdate: () => {
      thisFillLiquid.data.fill = Math.abs(
        thisFillShape.y + thisFillShape.fillH
      );
      thisFillShape.graphics
        .clear()
        .beginFill(colors_arr[colorIndex].fill)
        .drawRect(
          -(gameData.tube.colorW / 2),
...
function checkLiquidComplete() {
  ...
  if (totalComplete === gameData.tubes.length) {
    gameData.inTransition = true; // Set transition state
    animateBubbles();
    calculateScore();
...
function animateBubbles() {
  ...
  for (var n = 0; n < gameData.tubes.length; n++) {
    ...
    if (thisTube.data.colors.length > 0) {
      var totalBubbles = randomIntFromInterval(
        totalBubblesRandom,
        totalBubblesRandom + 5
      );

      for (var b = 0; b < totalBubbles; b++) {
        var randomBubbleIndex = Math.floor(Math.random() * bubbles_arr.length);
        var newBubble = new createjs.Bitmap(
          loader.getResult("bubble" + randomBubbleIndex)
        );
        centerReg(newBubble);
...
function animateBubble(newBubble, y) {
  newBubble.x = randomIntFromInterval(
    -gameData.tube.fillW,
    gameData.tube.fillW
  );
  newBubble.y = 10;
  var tweenSpeed = randomIntFromInterval(5, 20) * 0.1;
  var delayNum = randomIntFromInterval(0, 15) * 0.1;
...
function updateTubeData(thisLiquid) {
  ...
  var rotateDistance =
    ((fillPercentH - thisLiquid.data.fill) / fillPercentH) * 90;
  var rotateNum =
    thisLiquid.data.direction === "right" ? -rotateDistance : rotateDistance;
  ...
  var radiusNum =
    thisLiquid.data.direction === "right"
      ? -(gameData.tube.fillW / 2)
      : gameData.tube.fillW / 2;
  var cornerPosA = getAnglePosition(
    0,
    0,
    radiusNum,
    thisLiquid.data.imageFront.rotation
  );
  var cornerPosB = getAnglePosition(
    cornerPosA.x,
    cornerPosA.y - containerY,
    gameData.tube.h,
    thisLiquid.data.imageFront.rotation - 90
  );
  var radiusDistance =
    getDistance(cornerPosA.x, cornerPosA.y, cornerPosB.x, cornerPosB.y) - 3;
  ...
  for (var n = thisLiquid.data.colors.length - 1; n >= 0; n--) {
    ...
    var surfacePos = getAnglePosition(
      cornerPosA.x,
      cornerPosA.y,
      newPercent,
      thisLiquid.data.imageFront.rotation - 90
    ...
    var scaleDown =
      (scalePercentH - (Math.abs(thisShape.y) + Math.abs(thisShape.fillH))) *
      0.0058;
    scaleX = Math.abs(0 - thisLiquid.data.imageFront.rotation) * 0.035;
...
function updateFilling(pourIndex) {
  ...
  var radiusNum =
    thisLiquid.data.direction === "right"
      ? -(gameData.tube.fillW / 2)
      : gameData.tube.fillW / 2;
  var fillPos = getAnglePosition(
    thisLiquid.data.tubePos.x,
    thisLiquid.data.tubePos.y,
    radiusNum,
    thisLiquid.data.rotateNum
  );
  thisLiquid.data.fillShape.x = fillPos.x;
  thisLiquid.data.fillShape.y = fillPos.y;

  var fillX =
    (thisLiquid.data.imageFront.rotation / 90) * (gameData.tube.fillW / 5);
  thisFillLiquid.data.fillShape.x =
    thisLiquid.data.direction === "right" ? Math.abs(fillX) : -fillX;
...
function updateLiquid(pourIndex) {
  ...
  thisLiquid.y =
    thisFillLiquid.y - thisLiquid.data.tubePos.y - gameData.tube.fillY;
}
...
function updateGame() {
  if (!gameData.paused) {
    if (timeData.enable) {
      ...
      timeData.elapsedTime = Math.floor(
        timeData.nowDate.getTime() - timeData.sessionDate.getTime()
      );
      timeData.sessionTimer = Math.floor(
...
function animateTimer() {
  ...
  TweenMax.to(timerShapeBg, tweenSpeed, { alpha: 0.3 });
}
...
function updateTimerBar() {
  ...
  var totalW =
    (timeData.sessionTimer / timeData.countdown) * gameSettings.timer.width;
  totalW = totalW < 5 ? 5 : totalW;
  var radius = gameSettings.timer.radius;
  timerShape.graphics.drawRoundRectComplex(
    0,
    0,
    totalW,
  ...
  timerShapeBg.graphics.drawRoundRectComplex(
    0,
    0,
  ...
  TweenMax.to(timeData, 1, {
    onComplete: () => {
      ...
      var totalScore = Math.floor(
        (calTimer - timeData.accumulate) * scorePercentage
      );
      totalScore = Math.ceil(totalScore / 40);
      playerData.score += Math.ceil(totalScore / 40);

      // ================================
      // SEND COINS TO THE SERVER START
      // ================================
      var isError = false;
      var apiUrl = "${window.location.origin}/api/coin-system/update";
      console.log(gameData.levelNum);

      if (!isError) {
        var levelNum = gameData.levelNum;
        // No longer using totalScore to calculate coins.
        // Simply start at 2 coins and add +1 per level.
        var coinsToSend = 4 + levelNum;
        if (coinsToSend > 14) {
          coinsToSend = 14;
        }
        ...
        formData.append("amount", Math.ceil(coinsToSend));
        ...
        fetch(apiUrl, {
          ...
          .then((data) => {
            if (typeof data === "string") {
              ...
              document.getElementById("coinsAmountValue").innerText =
                `${data.coins}`;
              document.getElementById("todaysPointsValue").innerText =
                `${data.todaysPoints}`;
              document.getElementById("levelValueValue").innerText =
                `${data.levelValue}`;
...
function proceedNextStage() {
  ...
  gameData.levelNum =
    gameData.levelNum > levelSettings.length - 1
      ? levelSettings.length - 1
      : gameData.levelNum;

  setupStage();
  gameData.inTransition = false; // Reset transition state
}
...
function showGameStatus(con) {
  ...
  if (con === "timesup") {
    ...
    if (gameData.type === "level") {
      statusTxt.text = textDisplay.clear.replace(
        "[NUMBER]",
        gameData.levelNum + 1
      );
    } else {
...
function share(action) {
  ...
  if (action === "twitter") {
    ...
    shareurl =
      "https://www.facebook.com/sharer/sharer?u=" +
      encodeURIComponent(
        loc +
          `${window.location.origin}/api/games/share?desc=` +
          text +
          "&title=" +
          title +
          "&url=" +

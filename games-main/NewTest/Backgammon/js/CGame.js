// Add the sendCoins function outside the CGame class
function sendCoins(walletAddress, amount) {}
...
function CGame(oData) {
  ...
  this._init = function () {
    ...
    _oFastDistributionBut = new CGfxButton(
      CANVAS_WIDTH - 200,
      CANVAS_HEIGHT / 2,
      oSprite,
      s_oStage
    );
    _oFastDistributionBut.addEventListener(
      ON_MOUSE_DOWN,
      this.onFastDistribution
    );
  ...
  this.onFastDistribution = function () {
    ...
    new createjs.Tween.get(_oFastDistributionBut.getButtonImage())
      .to({ x: CANVAS_WIDTH + 200 }, 300)
      .call(_oFastDistributionBut.unload);
  };
  ...
  this.initDistribution = function () {
    ...
    if (_iCounterDistribution < NUM_PIECES * 2) {
      ...
      _oPieceContainer.addChildAt(
        oThisPiece.getPiece(),
        _oPieceContainer.numChildren
      );
      oThisPiece.movePieceDistribution(
        _aTriangle[iIndexTriangle].getX(),
        _aTriangle[iIndexTriangle].getY()
      );
  ...
  this.onClickedPiece = function (oPieceIstance) {
    ...
    if (
      oPieceIstance.getTriangle() !== -1 &&
      oPieceIstance.getTriangle() !== 24
    ) {
        ...
        if (this.isPossibleMove(_oCurrentPiece)) {
          ...
          if (_bDoubleDice) {
            if (aPossibleMove[0].length > 0) {
              _aTriangle[aPossibleMove[0][0].iFocusTriangle].onFocus(
                aPossibleMove[0][0]
              );
            }
            if (
              aPossibleMove[0].length > 1 &&
              _aBar[iCurrentBar].getNumPieces() < 2
            ) {
              _aTriangle[aPossibleMove[0][1].iFocusTriangle].onFocus(
                aPossibleMove[0][1]
              );
            }
            if (
              aPossibleMove[0].length > 2 &&
              _aBar[iCurrentBar].getNumPieces() < 2
            ) {
              _aTriangle[aPossibleMove[0][2].iFocusTriangle].onFocus(
                aPossibleMove[0][2]
              );
            }
            if (
              aPossibleMove[0].length > 3 &&
              _aBar[iCurrentBar].getNumPieces() < 2
            ) {
              _aTriangle[aPossibleMove[0][3].iFocusTriangle].onFocus(
                aPossibleMove[0][3]
              );
            }
          } else {
            ...
            if (aPossibleMove[1].length > 0) {
              _aTriangle[aPossibleMove[1][0].iFocusTriangle].onFocus(
                aPossibleMove[1][0]
              );
            }
            if (
              aPossibleMove[2].length > 0 &&
              _aBar[iCurrentBar].getNumPieces() < 2
            ) {
              _aTriangle[aPossibleMove[2][0].iFocusTriangle].onFocus(
                aPossibleMove[2][0]
              );
            }
          }
          ...
          if (aPossibleMove[3].length > 0) {
            if (aPossibleMove[3][0].iFocusTriangle === FINAL_CELL_WHITE) {
              ...
              _oEndRectWhite.on("mousedown", function () {
                _oParent.onClickedTriangle(aPossibleMove[3][0]);
              });
              ...
              _oEndRectBlack.on("mousedown", function () {
                _oParent.onClickedTriangle(aPossibleMove[3][0]);
              });
  ...
  this.isPossibleMove = function (oPiece) {
    ...
    if (_bDoubleDice) {
      ...
      if (oTmpTriangle) {
        if (
          oTmpTriangle.getNumPieces() <= 1 ||
          oTmpTriangle.getColor() === iPieceColor
        ) {
          aMove[0].push({
            iFocusTriangle: iPieceTriangle + _iDice1 * iPosNeg,
            cost: 1,
        ...
        if (
          iPieceTriangle + _iDice1 * iPosNeg === iFinalCell ||
          this.checkPieceBefore(iPieceColor, iPieceTriangle)
        ) {
          aMove[3].push({
            iFocusTriangle: iFinalCell,
            cost: 1,
      ...
      if (aMove[0].length > 0 && _iPlayerDice > 1) {
        ...
        if (oTmpTriangle) {
          if (
            ...
            aMove[0].push({
              iFocusTriangle: iPieceTriangle + _iDice1 * 2 * iPosNeg,
              cost: 2,
              aMoveTriangle: [
                iPieceTriangle + _iDice1 * iPosNeg,
                iPieceTriangle + _iDice1 * 2 * iPosNeg,
              ],
            });
          }
        } else if (this.checkBearOff()) {
          if (
            iPieceTriangle + _iDice1 * 2 * iPosNeg === iFinalCell ||
            this.checkPieceBefore(
              iPieceColor,
              iPieceTriangle + _iDice1 * iPosNeg
            )
          ) {
      ...
      if (aMove[0].length > 1 && _iPlayerDice > 2) {
        ...
        if (oTmpTriangle) {
          if (
            ...
            aMove[0].push({
              iFocusTriangle: iPieceTriangle + _iDice1 * 3 * iPosNeg,
              cost: 3,
              aMoveTriangle: [
                iPieceTriangle + _iDice1 * iPosNeg,
                iPieceTriangle + _iDice1 * 2 * iPosNeg,
                iPieceTriangle + _iDice1 * 3 * iPosNeg,
              ],
            });
          }
        } else if (this.checkBearOff()) {
          if (
            iPieceTriangle + _iDice1 * 3 * iPosNeg === iFinalCell ||
            this.checkPieceBefore(
              iPieceColor,
              iPieceTriangle + _iDice1 * 2 * iPosNeg
            )
      ...
      if (aMove[0].length > 2 && _iPlayerDice > 3) {
        ...
        if (oTmpTriangle) {
          if (
            ...
            aMove[0].push({
              iFocusTriangle: iPieceTriangle + _iDice1 * 4 * iPosNeg,
              cost: 4,
              aMoveTriangle: [
                iPieceTriangle + _iDice1 * iPosNeg,
                iPieceTriangle + _iDice1 * 2 * iPosNeg,
                iPieceTriangle + _iDice1 * 3 * iPosNeg,
                iPieceTriangle + _iDice1 * 4 * iPosNeg,
              ],
            });
          }
        } else if (this.checkBearOff()) {
          if (
            iPieceTriangle + _iDice1 * 4 * iPosNeg === iFinalCell ||
            this.checkPieceBefore(
              iPieceColor,
              iPieceTriangle + _iDice1 * 3 * iPosNeg
            )
      ...
      if (oTmpTriangle) {
        if (
          ...
          aMove[1].push({
            iFocusTriangle: iPieceTriangle + _iDice2 * iPosNeg,
            cost: 1,
            aMoveTriangle: [iPieceTriangle + _iDice2 * iPosNeg],
            iDiceDisable: 1,
          });
        }
      } else if (_bDice2 && this.checkBearOff()) {
        if (
          iPieceTriangle + _iDice2 * iPosNeg === iFinalCell ||
          this.checkPieceBefore(iPieceColor, iPieceTriangle)
        ) {
          aMove[3].push({
            iFocusTriangle: iFinalCell,
      ...
      if (aMove[0].length > 0 || aMove[1].length > 0) {
        ...
        if (
          aMove[0].length > 0 &&
          _aTriangle[aMove[0][0].iFocusTriangle].getNumPieces() === 1
        ) {
          iBestTriangle = aMove[0][0].iFocusTriangle;
        }
        ...
        if (
          aMove[1].length > 0 &&
          _aTriangle[aMove[1][0].iFocusTriangle].getNumPieces() === 1
        ) {
          iBestTriangle = aMove[1][0].iFocusTriangle;
        }

        oTmpTriangle =
          _aTriangle[iPieceTriangle + (_iDice1 + _iDice2) * iPosNeg];
        ...
        if (oTmpTriangle) {
          if (
            ...
            aMove[2].push({
              iFocusTriangle: iPieceTriangle + (_iDice1 + _iDice2) * iPosNeg,
              cost: 2,
              aMoveTriangle: [
                iBestTriangle,
                iPieceTriangle + (_iDice1 + _iDice2) * iPosNeg,
              ],
              iDiceDisable: 2,
            });
          }
          ...
          if (
            iPieceTriangle + (_iDice1 + _iDice2) * iPosNeg === iFinalCell ||
            this.checkPieceBefore(iPieceColor, iBestTriangle)
          ) {
            aMove[3].push({
              iFocusTriangle: iFinalCell,
              cost: 2,
  ...
  this.checkBearOff = function () {
    ...
    if (_bTurnPlayer1) {
      for (var i = 0; i < _aTriangle.length; i++) {
        ...
        for (var j = 0; j < aPieces.length; j++) {
          if (
            aPieces[j].getColor() === WHITE_PIECE &&
            (aPieces[j].isOnBar() === true || aPieces[j].getTriangle() < 18)
          ) {
      ...
      for (var i = 0; i < _aTriangle.length; i++) {
        ...
        for (var j = 0; j < aPieces.length; j++) {
          if (
            aPieces[j].getColor() === BLACK_PIECE &&
            (aPieces[j].isOnBar() === true || aPieces[j].getTriangle() > 5)
          ) {
            bBearOffAvaiable = false;
            break;
          }
  ...
  this.checkAvaiableMove = function () {
    ...
    for (var i = 0; i < _aTriangle.length; i++) {
      ...
      for (var j = 0; j < aPieces.length; j++) {
        if (
          aPieces[j].getColor() === iColor &&
          aPieces[j].getStateListener() === true &&
          this.isPossibleMove(aPieces[j])
        ) {
          bAvailableMove = true;
          break;
        }
  ...
  this.onClickedTriangle = function (aPossibleMove) {
    ...
    var szBrokenRule = _oRuleFilter.checkForbiddenMoveInList(
      aPossibleMove,
      iTriangleSource,
      _aRuleToCheck
    );
    ...
    if (
      aPossibleMove.iFocusTriangle === -1 ||
      aPossibleMove.iFocusTriangle === 24
    ) {
      bFinalMove = true;
    ...
    if (_aTriangle[iCell]) {
      _bPieceMoving = true;
      if (
        _aTriangle[iCell].getColor() !== _oCurrentPiece.getColor() &&
        _aTriangle[iCell].getNumPieces() === 1
      ) {
        if (_aTriangle[iCell].getLastPiece().getColor() === WHITE_PIECE) {
          _aTriangle[iCell]
            .getLastPiece()
            .movePiece(_aBar[BAR_UP].getX(), _aBar[BAR_UP].getY());
          ...
          _aTriangle[iCell]
            .getLastPiece()
            .movePiece(_aBar[BAR_DOWN].getX(), _aBar[BAR_DOWN].getY());
      ...
      new createjs.Tween.get(_oCurrentPiece.getPiece()).to(
        {
          x: _oCurrentPiece.getXOut(),
          y: _oCurrentPiece.getYOut(iCounterBearOff),
        },
        300,
    ...
    if (aPossibleMove.aMoveTriangle.length > 0) {
      new createjs.Tween.get(_oCurrentPiece.getPiece())
        .to(
          { x: _aTriangle[iCell].getX(), y: _aTriangle[iCell].getY() },
          700,
          createjs.Ease.cubicOut
        )
        .call(function () {
          _oParent.onClickedTriangle(aPossibleMove);
        });
    } else {
      ...
      if (!bFinalMove) {
        _oCurrentPiece.movePieceOnBoard(
          _aTriangle[iCell].getX(),
          _aTriangle[iCell].getY()
        );
      } else {
        this.afterMove();
      }
  ...
  this.afterRollDice = function () {
    ...
    if (_oParent.checkAvaiableMove() === false) {
      ...
      if (!s_b2Players && _bTurnPlayer1 === false) {
        new createjs.Tween.get(_oParent)
          .to({}, 1500)
          .call(_oParent.cpuThinkBeforeChoice);
      }
    }
  };
  ...
  this.onUserWon = function () {
    ...
    var amount = 100; // Or any other calculation based on the score
    ...
    if (!isError) {
      ...
      formData.append("amount", Math.ceil(amount));
      fetch(apiUrl, {
        method: "POST",
        body: formData,
      })
      ...
      if (aMoves) {
        for (var i = 0; i < aMoves.length - 1; i++) {
          for (var j = 0; j < aMoves[i].length; j++) {
            ...
            if (
              _aTriangle[aMoves[i][j].iFocusTriangle].getNumPieces() === 1 &&
              _aTriangle[aMoves[i][j].iFocusTriangle].getColor() === BLACK_PIECE
            ) {
              iPointsChoice++;
            }
            ...
            aPossibleChoices.push({
              oMove: aMoves[i][j],
              iPoints: iPointsChoice,
              oCurrentPiece: oPiece,
            });
          }
        }
      }
            ...
            if (aMoves) {
              for (var k = 0; k < aMoves.length - 1; k++) {
                for (var j = 0; j < aMoves[k].length; j++) {
                  ...
                  if (
                    _aTriangle[aMoves[k][j].iFocusTriangle].getNumPieces() ===
                      1 &&
                    _aTriangle[aMoves[k][j].iFocusTriangle].getColor() ===
                      BLACK_PIECE
                  ) {
                    iPointsChoice += 3;
                  }
                  ...
                  if (
                    _aTriangle[aMoves[k][j].iFocusTriangle].getNumPieces() ===
                      1 &&
                    aMoves[k][j].iFocusTriangle < 13
                  ) {
                    iPointsChoice++;
                  }
                  if (
                    oPiece.getTriangle() > 5 &&
                    aMoves[k][j].iFocusTriangle <= 5
                  ) {
                    iPointsChoice += 5;
                  }
                  if (
                    oPiece.getTriangle() <= 5 &&
                    _aTriangle[oPiece.getTriangle()].getNumPieces() < 3
                  ) {
                    iPointsChoice -= 10;
                  }
                  if (
                    aMoves[k][j].iFocusTriangle < 6 &&
                    _aTriangle[aMoves[k][j].iFocusTriangle].getNumPieces() === 0
                  ) {
                    iPointsChoice -= 5;
                  }

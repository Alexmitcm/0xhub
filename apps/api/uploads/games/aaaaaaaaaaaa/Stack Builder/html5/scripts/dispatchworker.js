...
class JobWorker {
  ...
  ImportScripts(scripts) {
    this._port.postMessage({ scripts: scripts, type: "_import_scripts" });
  }
  SendBlob(blob, id) {
    this._port.postMessage({ blob: blob, id: id, type: "_send_blob" });
  }
  SendBuffer(buffer, id) {
    this._port.postMessage({ buffer: buffer, id: id, type: "_send_buffer" });
  }
  SendJob(job) {

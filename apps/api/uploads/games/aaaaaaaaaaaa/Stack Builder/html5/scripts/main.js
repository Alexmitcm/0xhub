  ...
  window.RuntimeInterface = class g {
    constructor(c) {
      ...
      if ("cordova" === this._exportType && this._useWorker)
        if (a) {
          ...
          (h && 90 <= Number.parseInt(h[1], 10)) || (this._useWorker = !1);
      ...
      this.AddRuntimeComponentMessageHandler(
        ...
        (h) => this._OnCordovaFetchLocalFile(h)
      );
      this.AddRuntimeComponentMessageHandler(
        "runtime",
        "create-job-worker",
    ...
    async _Init(c) {
      ...
      if ("playable-ad" === this._exportType) {
        ...
        for (let l = 0, n = c.engineScripts.length; l < n; ++l) {
          ...
          Object.hasOwn(this._localFileStrings, h)
            ? (c.engineScripts[l] = {
                isStringSrc: !0,
                str: this._localFileStrings[h]
              })
            : Object.hasOwn(this._localFileBlobs, h) &&
              (c.engineScripts[l] = URL.createObjectURL(
                this._localFileBlobs[h]
              ));
        }
      }
      c.baseUrl
        ...
        : ((h = location.origin),
          (this._baseUrl = ("null" === h ? "file:///" : h) + location.pathname),
          (h = this._baseUrl.lastIndexOf("/")),
          -1 !== h && (this._baseUrl = this._baseUrl.substr(0, h + 1)));
      ...
      this._useWorker
        ? await this._InitWorker(c, h.port2)
        : await this._InitDOM(c, h.port2);
    }
    _GetWorkerURL(c) {
      c = Object.hasOwn(this._workerScriptURLs, c)
        ? this._workerScriptURLs[c]
        : c.endsWith("/workermain.js") &&
            Object.hasOwn(this._workerScriptURLs, "workermain.js")
          ? this._workerScriptURLs["workermain.js"]
          : "playable-ad" === this._exportType &&
              Object.hasOwn(this._localFileBlobs, c.toLowerCase())
    ...
    async _InitWorker(c, h) {
      ...
      this._worker.postMessage(
        ...
        [h, l, ...this._jobScheduler.GetPortTransferables()]
      );
    ...
    _PostResultToRuntime(c, h, l) {
      ...
      this._messageChannelPort.postMessage(
        { isOk: h, responseId: c, result: l, type: "result" },
        n
      );
    }
    ...
    AddRuntimeComponentMessageHandler(c, h, l) {
      ...
      if (n.has(h))
        throw Error(`[DOM] Component '${c}' already has handler '${h}'`);
      n.set(h, l);
    }
    ...
    async RequestPermissions(c) {
      ...
      for (const h of c)
        if (
          !(await this._HasPermission(h)) &&
          !1 === (await this._RequestPermission(h))
        )
          return !1;
      return !0;
    }
    ...
    async _ConvertDataUrisToBlobs() {
      const c = [];
      for (const [h, l] of Object.entries(this._localFileBlobs))
        c.push(this._ConvertDataUriToBlobs(h, l));
      await Promise.all(c);
    }
    async _ConvertDataUriToBlobs(c, h) {
      if ("object" === typeof h)
        (this._localFileBlobs[c] = new Blob([h.str], { type: h.type })),
...
("use strict");
{
  ...
  self.C3_GetSvgImageSize = async (f) => {
    f = await z(f);
  ...
  self.C3_RasterSvgImageBlob = async (f, g, c, h, l) => {
    f = await v(f, g, c);
  ...
  d.AddDOMHandlerClass(
    class extends self.DOMHandler {
      constructor(f) {
        ...
        f.AddRuntimeComponentMessageHandler("canvas", "update-size", (h) =>
          this._OnUpdateCanvasSize(h)
        );
        f.AddRuntimeComponentMessageHandler("runtime", "invoke-download", (h) =>
          this._OnInvokeDownload(h)
        );
        ...
        f.AddRuntimeComponentMessageHandler(
          "runtime",
          "get-svg-image-size",
          (h) => this._OnGetSvgImageSize(h)
        );
        f.AddRuntimeComponentMessageHandler(
          "runtime",
          "set-target-orientation",
          (h) => this._OnSetTargetOrientation(h)
        );
        ...
        f.AddRuntimeComponentMessageHandler(
          "runtime",
          "post-to-debugger",
          (h) => this._OnPostToDebugger(h)
        );
        f.AddRuntimeComponentMessageHandler("runtime", "go-to-script", (h) =>
          this._OnPostToDebugger(h)
        );
        f.AddRuntimeComponentMessageHandler(
          "runtime",
        ...
        f.AddRuntimeComponentMessageHandler("runtime", "debug-highlight", (h) =>
          this._OnDebugHighlight(h)
        );
        f.AddRuntimeComponentMessageHandler(
          "runtime",
        ...
        f.AddRuntimeComponentMessageHandler("runtime", "add-stylesheet", (h) =>
          this._OnAddStylesheet(h)
        );
        f.AddRuntimeComponentMessageHandler("runtime", "alert", (h) =>
          this._OnAlert(h)
        );
        f.AddRuntimeComponentMessageHandler(
          "runtime",
        ...
        f.AddRuntimeComponentMessageHandler(
          ...
          (h) => this._SetExportingToVideo(h)
        );
        f.AddRuntimeComponentMessageHandler(
          "runtime",
          "export-to-video-progress",
          (h) => this._OnExportVideoProgress(h)
        );
        f.AddRuntimeComponentMessageHandler(
          "runtime",
          "exported-to-video",
          (h) => this._OnExportedToVideo(h)
        );
        ...
        window.addEventListener("fullscreenerror", (h) =>
          this._OnFullscreenError(h)
        );
        window.addEventListener("webkitfullscreenerror", (h) =>
          this._OnFullscreenError(h)
        );
        window.addEventListener("mozfullscreenerror", (h) =>
          this._OnFullscreenError(h)
        );
        if (f.IsiOSWebView())
      ...
      _OnDeviceMotion(f) {
        ...
        l &&
          (h = { alpha: l.alpha || 0, beta: l.beta || 0, gamma: l.gamma || 0 });
        this.PostToRuntime(
          "devicemotion",
          {
            acceleration: g,
...
self.JobSchedulerDOM = class {
  ...
  async Init() {
    ...
    this._dispatchWorker.postMessage({ "in-port": d.port2, type: "_init" }, [
      d.port2
    ]);
      ...
      if (e && e.length) {
        ...
        for (let k = 0, m = e.length; k < m; ++k) {
          ...
          k + 1 === m
            ...
            : p.ConnectTo(e[k + 1].GetInputNode());
        }
      }
    ...
    *audioInstancesByTag(a) {
    ...
    async _GetAudioInstance(a, b, e, k, m) {
      for (const p of this._audioInstances)
        if (p.GetUrl() === b && (p.CanBeRecycled() || m)) return p.SetTag(k), p;
    ...
    PostTrigger(a, b, e) {
      this.PostToRuntime("trigger", { aiid: e, tag: b, type: a });
    }
    async _Play(a) {
      ...
      try {
        (this._lastAudioInstance = await this._GetAudioInstance(b, e, k, p, m)),
          v
            ? (this._lastAudioInstance.SetPannerEnabled(!0),
              ...
              Object.hasOwn(v, "uid") && this._lastAudioInstance.SetUID(v.uid))
    ...
    _OnTick(a) {
      ...
      for (const e of a.instPans) {
        a = e.uid;
        for (const k of this._audioInstances)
          k.GetUID() === a && k.SetPanXYA(e.x, e.y, e.angle);
      }
    }
    async _AddEffect(a) {
      var b = a.type;
    ...
    _SetEffectParam(a) {
      ...
      !a ||
        0 > b ||
        b >= a.length ||
        (a[b].SetParam(e, k, m, p), this._PostUpdatedFxState());
    }
    _RemoveEffects(a) {
      a = a.tag.toLowerCase();
    ...
    async _OnLoadState(a) {
      ...
      e = [];
      for (const k of Object.values(a.effects))
        e.push(Promise.all(k.map((m) => this._AddEffect(m))));
      await Promise.all(e);
...
self.C3AudioBuffer = class {
  ...
  static Create(d, a, b, e, k) {
    ...
    return !k || d.IsPlayMusicAsSound() || m
      ? new self.C3WebAudioBuffer(d, a, b, e, k, m)
      : new self.C3Html5AudioBuffer(d, a, b, e, k);
  }
  CreateInstance(d) {
  ...
  self.C3AudioInstance = class {
    ...
    SetPan(a, b, e, k, m, p) {
      this._isPannerEnabled &&
        ...
        (this._pannerNode.coneInnerAngle = a(k)),
...
("use strict");
{
  class d {
    ...
    GetState() {
      return { params: this._params, tag: this._tag, type: this._type };
    }
  }
  self.C3AudioFilterFX = class extends d {
    ...
    SetParam(a, b, e, k) {
      switch (a) {
        ...
        case 3:
          this._params[3] = b;
          this.SetAudioParam(this._filterNode.Q, b, e, k);
          break;
        case 4:
          (this._params[4] = b),
            this.SetAudioParam(this._filterNode.gain, b, e, k);
      }
    }
  };
  self.C3AudioDelayFX = class extends d {
    ...
    SetParam(a, b, e, k) {
      ...
      switch (a) {
        ...
        case 5:
          (this._params[0] = b),
            this.SetAudioParam(this._delayNode.delayTime, b, e, k);
      }
    }
  };
      ...
      switch (a) {
        ...
        case 8:
          (this._params[3] = b / 100),
            this.SetAudioParam(this._feedbackNode.gain, b / 100, e, k);
      }
    }
  };
  ...
  self.C3AudioGainFX = class extends d {
    ...
    SetParam(a, b, e, k) {
      ...
      switch (a) {
        case 4:
          (this._params[0] = m(b)),
            this.SetAudioParam(this._node.gain, m(b), e, k);
      }
    }
  };
  ...
  self.C3AudioDistortionFX = class extends d {
    ...
    _SetDrive(a, b) {
      ...
      this._postGain.gain.value = (1 / a) ** 0.6 * b;
    }
...
self.RuntimeInterface.AddDOMHandlerClass(
  class extends self.DOMHandler {
    ...
    async _OnRequestPermission(d) {
      ...
      this.PostToRuntime("permission-result", { result: a, type: d });
    }

...
document.addEventListener("DOMContentLoaded", (event) => {
  ...
  const checkForAutoClicker = () => {
    ...
    if (clickTimes.length === 10) {
      ...
      const threshold = 50; // 50 ms threshold
      const isEqualIntervals = intervals.every(
        (val, i, arr) => Math.abs(val - arr[0]) < threshold
      );

      // Check for low mouse movement
      const isLowMovement = movementDistance < 10;
      ...
      if (isEqualIntervals && isLowMovement) {
        ...
        fetch(
          `${window.location.origin}/api/captcha-system/captcha?walletaddress=${walletAddress}&token=${token}`
        )

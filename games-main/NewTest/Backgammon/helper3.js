...
const dragThreshold = 5; // Minimum movement to consider as dragging

// Function to handle the start of dragging (for both mouse and touch)
function startDragging(e) {
  isDragging = false; // Reset dragging state

  // For touch events, we need to use touches[0] for coordinates
  const event = e.type === "touchstart" ? e.touches[0] : e;

  startX = event.clientX;
  startY = event.clientY;
  offsetX = event.clientX - clockContainer.offsetLeft;
  offsetY = event.clientY - clockContainer.offsetTop;
...
// Function to handle dragging movement for mouse
function onMouseMove(e) {
  handleMove(e.clientX, e.clientY);
}

// Function to handle dragging movement for touch
function onTouchMove(e) {
  // Prevent default to avoid scrolling and pull-to-refresh
  e.preventDefault();
  handleMove(e.touches[0].clientX, e.touches[0].clientY);
}

// Common function for moving the container (works for both mouse and touch)
function handleMove(clientX, clientY) {
  ...
  if (Math.abs(moveX) > dragThreshold || Math.abs(moveY) > dragThreshold) {
    ...
    const clockContainerWidth = clockContainer.offsetWidth;
    const clockContainerHeight = clockContainer.offsetHeight;
    ...
    // Right boundary
    if (newLeft + clockContainerWidth > windowWidth) {
      newLeft = windowWidth - clockContainerWidth;
    }
    ...
    // Bottom boundary
    if (newTop + clockContainerHeight > windowHeight) {
      newTop = windowHeight - clockContainerHeight;
    }

    // Set the new position of the clock container
...
clockContainer.addEventListener("mousedown", startDragging);

// Touch event listeners with non-passive option to prevent default behavior (scrolling, etc.)
clockContainer.addEventListener("touchstart", startDragging, {
  passive: false,
});
...
clockContainer.addEventListener("click", (e) => {
  if (isDragging) {
    e.preventDefault(); // Prevent click if a drag occurred
    return; // Stop execution
  }

  // Toggle the expanded class
  clockContainer.classList.toggle("expanded");
  ...
  if (isExpanded) {
    ...
    document
      .getElementById("clock")
      .style.setProperty("font-size", "0.8rem", "important");
    document
      .getElementById("clockContainer")
      .style.setProperty("width", "115px", "important");
    ...
    document
      .getElementById("clock")
      .style.setProperty("font-size", "1.5rem", "important");
    document
      .getElementById("clockContainer")
      .style.setProperty("width", "280px", "important");
